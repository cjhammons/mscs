...
class Router:
    ...
    def update(self, rt):
        ...
        # Check if we already have a route from this specific neighbor
        found = False
        ...
        # If we didn't find the neighbor, add the new route
        if not found:
    ...
    def next_hop(self, ipaddr):
        ...
        target_ip_int = ip_to_int(ipaddr)
        
        best_prefix_len = -1
        best_routes_list = None
        
        for prefix_key, routes in self.rib.items():
            prefix_ip_str, prefix_len_str = prefix_key.split('/')
            prefix_len = int(prefix_len_str)
            prefix_ip_int = ip_to_int(prefix_ip_str)
            ...
            if (target_ip_int & mask) == (prefix_ip_int & mask):
                if prefix_len > best_prefix_len:
                    best_prefix_len = prefix_len
                    best_routes_list = routes 

        if best_routes_list is None:
            return None 

        best_route = None 
        min_path_len = float('inf')

        for route in best_routes_list:
            current_path_len = len(route.path)
